...
internal static class CodeGenerator
{
    ...
    private static string GenerateDeepCloneExtensions(EquatableArray<ClassInfo> classInfos)
    {
        ...
        foreach (var classInfo in classInfos)
        {
            ...
            {
                GenerateCloneInternalMethod(classInfo, classInfos, builder);
            }
        }

        // Generate special collection clone methods
        GenerateCollectionCloneMethods(classInfos, builder);
    ...
    private static void GenerateCloneInternalMethod(ClassInfo classInfo, EquatableArray<ClassInfo> allClassInfos, IndentedStringBuilder builder)
    {
    ...
    private static void GenerateCollectionCloneMethods(EquatableArray<ClassInfo> classInfos, IndentedStringBuilder builder)
    {
        ...
        foreach (var classInfo in classInfos)
        {
            foreach (var prop in classInfo.Properties)
            {
                ...
                if (typeFullName.EndsWith("[]"))
                {
                    ...
                    {
                        generatedMethods.Add(methodName);
                        GenerateArrayCloneMethod(elementType, methodName, classInfos, builder);
                    }
                }
            }
        }
    ...
    private static string GenerateListCloneMethod(string listType, string innerType, string methodName, EquatableArray<ClassInfo> allClassInfos, IndentedStringBuilder builder)
    {
        ...
        if (isImmutable)
        {
            builder.Append($"            return new {listType}(original);");
        }
        else
        {
    ...
    private static string GenerateDictionaryCloneMethod(string dictType, string methodName, EquatableArray<ClassInfo> allClassInfos, IndentedStringBuilder builder)
    {
        ...
        {
            builder.Append($"            return new {dictType}(original);");
        }
        else
        {
            builder.Append($"            var dict = new {dictType}(original.Count);");
    ...
    private static void GenerateArrayCloneMethod(string elementType, string methodName, EquatableArray<ClassInfo> allClassInfos, IndentedStringBuilder builder)
    {
